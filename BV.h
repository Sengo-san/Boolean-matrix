...
class BooolVector
{
	UC* bv;
	int n, m;		
				//n - длина булева вектора (реальное количество бит)
			   //m - кол-во эл-тов в массиве bv (количесво отведенных байт)
			   //m=n/8 если n%8=0;  m=n/8+1, если n%8!=0 
	int Input(const char*, int, int, int); 		//формирование эл-та массива bv (формирование байта)
	void Output(int, int);			//вывод одного эл-та массива bv (вывод одного байта)

public:
	BooolVector();
	BooolVector(int N); 				//N - длина булева вектора
	BooolVector(const char* s);
